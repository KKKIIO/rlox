...
fn line_comment<'a, E: ParseError<Span<'a>> + ContextError<Span<'a>>>(
    ...
    recognize(tuple((tag("//"), cut(not_line_ending), cut(line_ending))))(input)
}
...
mod test {
    ...
    use super::*;
    #[test]
    fn test_line_comment() -> Result<(), Box<dyn Error>> {
        let input = Span::new("// This is a comment\n");
        let (rest, res) = line_comment::<nom::error::Error<Span>>(input)?;
